...
// Clear all existing date ideas from the database
async function clearDateIdeas() {
  console.log("Clearing existing date ideas...");
  const dateIdeasRef = collection(db, "date_ideas");
  const snapshot = await getDocs(dateIdeasRef);

  const deletePromises = snapshot.docs.map((doc) => deleteDoc(doc.ref));
  await Promise.all(deletePromises);

  console.log(`Cleared ${snapshot.docs.length} existing date ideas`);
}
...
// Seed date ideas collection with all love language combinations (for testing - call this from your app)
export async function seedDateIdeas() {
  if (!auth.currentUser) {
    throw new Error("Must be authenticated");
  }

  console.log("Starting to seed date ideas from authenticated user...");

  try {
    // Clear existing date ideas first
    await clearDateIdeas();

    let totalSeeded = 0;

    for (const [combination, dates] of Object.entries(dateIdeas)) {
      console.log(`Seeding dates for: ${combination}`);
